...
author: "Avinash Pawar(devopstechie.com :: https://github.com/avinash6784)"
...
options:
  ...
  ldap_user:
   description:
    - LDAP username when auth_method is ldap

  ldap_pass:
   description:
    - LDAP password when auth_method is ldap
...
def main():
	...
	module = AnsibleModule(
		argument_spec  = dict(
			...
			root_token = dict(required=False, type='str', no_log=True),
			unseal_key = dict(required=False, type='str', no_log=True),
			...
			action = dict(required=False, default='status', type='str', choice=["status", "seal-status", "seal", "unseal", "read", "write"])
			)
		)
